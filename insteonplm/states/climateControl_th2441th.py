...
from insteonplm.states.thermostat import Temperature, Humidity, SystemMode, FanMode, HeatSetPoint, CoolSetPoint


class Temperature_2441th(Temperature):
...
class Humidity_2441th(Humidity):

    def _ext_status_received(self, msg):
        humid = msg.userdata['d8']
        self._update_subscribers(humid)

class SystemMode_2441th(SystemMode):
...
class CoolSetPoint_2441th(CoolSetPoint):
...
class HeatSetPoint_2441th(HeatSetPoint):
